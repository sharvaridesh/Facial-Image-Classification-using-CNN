...
def main(unused_argv):
    ...
    for i in glob.glob ('*.jpg'):
        obj = cv2.imread (i, 0)
        obj = cv2.equalizeHist(obj)
        obj_flatten = obj.flatten()
        train_face[a, :] = obj_flatten
        a = a + 1
    ...
    face_classifier.train (input_fn=train_input_fn, steps=5000, hooks=[logging_hook])
